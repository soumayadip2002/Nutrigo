<?php
    ...
    $title = $_SESSION['add-blogs-data']['title'] ?? null;
    $content = $_SESSION['add-blogs-data']['content'] ?? null;
    unset($_SESSION['add-blogs-data']);
?>

<section class="form_container" style="margin-top:6rem">
    <?php if(isset($_SESSION['add-blogs'])) {?>
        <div class="alert error">
            <i class="fa-solid fa-triangle-exclamation"></i>
            <p>
               <?= $_SESSION['add-blogs'];
                    unset($_SESSION['add-blogs']);
                ?>
            </p>
            <i class="fa-solid fa-circle-xmark" id="alert_close"></i>
    	</div>
    <?php } ?>
    <h3 class="heading">add <span>blog</span></h3>
    <form action="./add_blog-logic.php" class="add_blog form_inline" method="post" enctype="multipart/form-data">
        <input type="text" name="title" value="<?= $title ?>" placeholder="enter title..." required>
        <br>
        <textarea rows="20" name="content" id="myTextarea" placeholder="add body....." ><?= $content ?></textarea>
