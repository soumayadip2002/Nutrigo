...
(function($){
     ...
     * @private
     */
    var _initZoom = function(target, sWidth, sHeight,options){
        var zoomLevel = options && options.zoomLevel ? options.zoomLevel : 1; // Add zoom level option
        var zoomWidth = sWidth * zoomLevel;
        var zoomHeight = sHeight * zoomLevel;
        var $zoom = $("<div />").addClass("zoom-selector").width(zoomWidth).height(zoomHeight);
        target.after($zoom);
